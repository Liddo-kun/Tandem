...
- Folds mobile session title, context usage, and overflow actions into the Session tab with a 65%/35% Session/Changes split, hides the duplicate timeline title bar on phones except while renaming, and defers native-mobile delete confirmation until after the non-modal menu closes. Files: `packages/app/src/pages/session.tsx`, `packages/app/src/pages/session/message-timeline.tsx`, `packages/app/src/pages/session/message-timeline-mobile-tabs.tsx`, `packages/app/src/pages/session/session-title-actions.tsx`.
