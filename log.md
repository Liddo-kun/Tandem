...
- Adds mobile-aware reversed timeline scrolling, bottom detection, dynamic overflow anchoring, contained prompt/scroll-view overscroll, prompt and nested scroll gesture protection, composer touchmove containment while the keyboard is open, scroll-view keyboard endpoints for iOS/Android positive scroll offsets, and native-mobile timeline turns that render directly instead of using virtualized or `content-visibility`/`contain-intrinsic-size` placeholders. Files: `packages/app/src/components/prompt-input.tsx`, `packages/app/src/pages/session.tsx`, `packages/app/src/pages/session/message-timeline.tsx`, `packages/app/src/pages/session/message-timeline.data.ts`, `packages/app/src/pages/session/composer/session-composer-region.tsx`, `packages/ui/src/components/scroll-view.tsx`, `packages/ui/src/components/scroll-view.css`, `packages/ui/src/hooks/create-auto-scroll.tsx`.
