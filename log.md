...
- Adds mobile-aware reversed timeline scrolling, bottom detection, dynamic overflow anchoring, contained prompt/scroll-view overscroll, prompt and nested scroll gesture protection, scroll-view keyboard endpoints for iOS/Android positive scroll offsets, and native-mobile timeline turns that skip per-turn `content-visibility`/`contain-intrinsic-size` placeholders. Files: `packages/app/src/components/prompt-input.tsx`, `packages/app/src/pages/session.tsx`, `packages/app/src/pages/session/message-timeline.tsx`, `packages/ui/src/components/scroll-view.tsx`, `packages/ui/src/components/scroll-view.css`, `packages/ui/src/hooks/create-auto-scroll.tsx`.
