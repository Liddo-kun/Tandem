...
- Build + install on the tablet itself with one command from repo root: `bun run tandem:tablet -- --install` (native aarch64 signed release APK, same keystore as `tandem:release`; pass `--debug` for a debug build; auto-connects ADB, prefers `127.0.0.1:5555`; add `--overwrite` to replace a signature-mismatched install). On a fresh chroot run `bun run tandem:tablet -- --setup` once to install the arm64 toolchain (NDK + glibc build-tools + JDK + Rust). Rationale and pinned sources: `packages/android/apkbuildontablet.md`.
...
- Side-by-side variant: from `packages/android` run `bun run install:y700 -- -Name <name>` — builds a parallel-installable optimized release APK (id `ai.opencode.android.<name>`) and installs it without touching the main app. It uses the normal release profile/signing, self-bootstraps the toolchain env, and auto-resolves/connects the device, so just run it directly: no `tee`/`rg` wrapper, no ADB pre-check, no `-Device` needed on the tablet (pass `-Device <serial>` only to override). Existing debug-signed variants need a one-time `-Overwrite` migration, which erases only that variant's app data.
