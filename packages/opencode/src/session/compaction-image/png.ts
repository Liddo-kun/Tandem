...
// Adapted from pxpipe (github.com/teamchong/pxpipe) src/core/png.ts, MIT License,
// Copyright (c) 2026 claude-image-proxy contributors.
// Minimal PNG encoder (grayscale + RGB, 8-bit, filter=None, single IDAT). Pure
// Uint8Array + CompressionStream — no native canvas dependency, runs under Bun on
// the tablet. Output is deterministic for identical input (no timestamps in pixels).
...
async function deflateZlib(input: Uint8Array): Promise<Uint8Array> {
  // 'deflate' = RFC 1950 zlib-wrapped — what PNG IDAT needs ('deflate-raw' would be wrong).
...
export async function encodeGrayPng(pixels: Uint8Array, width: number, height: number): Promise<Uint8Array> {
  ...
  return concat([PNG_SIGNATURE, chunk("IHDR", ihdr), chunk("IDAT", compressed), chunk("IEND", new Uint8Array(0))])
}

/** Encode an RGB (3 bytes/pixel) buffer as PNG bytes (colorType 2 = truecolor). length = width × height × 3. */
