...
import { getDeleteWordRange, getEditorText, getSelectionRange, setSelectionRange } from "./prompt-input/editor-dom"
...
export type PromptInputV2ComposerController = PromptInputV2Interaction & {
  ...
  // UPSTREAM-DIVERGENCE: Tandem composer extras (cache-health context button, RePrompt toggle,
  // native mobile mic) need session identity and the context-tab toggle at render time.
  readonly tandem: {
...
export function PromptInputV2Composer(props: PromptInputV2ComposerProps) {
  ...
  const sessionMessages = createMemo(() => {
    const id = props.controller.tandem.sessionID()
    ...
    <div class="flex flex-col gap-3">
      <PromptInputV2
        ...
        leadingControls={
          ...
          <Show when={props.controller.state.mode === "normal" && mobilePlatform(platform) && platform.startVoiceInput}>
            <TooltipV2 placement="top" value="Voice input">
              <IconButton
        ...
        modelControl={
          <PromptInputV2ModelControl
            ...
            modelName={promptComposerModelName(
...
export function usePromptInputV2Controller(props: PromptInputV2ControllerProps): PromptInputV2ComposerController {
  ...
  // UPSTREAM-DIVERGENCE: Tandem's context-token button toggles the session context tab;
  // named openContextTab to avoid upstream's add-menu openContext.
  ...
  const controller = createPromptInputV2Controller({
    ...
    view: {
      ...
      // UPSTREAM-DIVERGENCE: mobile keyboards have no Shift+Enter, so Enter inserts a newline
      // instead of submitting; the send button submits.
      onKeyDown: (event) => {
        if (!mobilePlatform(platform)) return
  ...
  // UPSTREAM-DIVERGENCE: Tandem composer extras and native mobile keyboard/voice behavior.
  Object.defineProperty(controller, "tandem", {
    get: () => ({ sessionID: () => props.controls.session.id, openContextTab }),
  })
  useMobileEditorEvents(() => editor, controller)
  return controller as PromptInputV2ComposerController
}

const mobilePlatform = (platform: ReturnType<typeof usePlatform>) =>
...
// UPSTREAM-DIVERGENCE: native mobile voice transcription and keyboard delete-word events, plus
// Enter inserting a newline instead of submitting (mobile keyboards have no Shift+Enter).
function useMobileEditorEvents(editor: () => HTMLDivElement | undefined, controller: PromptInputV2Interaction) {
  const platform = usePlatform()
  if (!mobilePlatform(platform)) return

  const reparse = (element: HTMLDivElement) =>
  ...
  createEffect(() => {
    const handleTranscription = (event: Event) => {
      ...
      if (!element) return
      element.focus()
      controller.onCursor(controller.value().length)
      controller.addPart({ type: "text", content: detail.text, start: 0, end: 0 })
    ...
    const handleKeyboardDeleteWord = () => {
      ...
      const deleteRange = getDeleteWordRange(getEditorText(element), getSelectionRange(element))
      if (!deleteRange) return

      element.focus()
      setSelectionRange(element, range, deleteRange.start, deleteRange.end)
