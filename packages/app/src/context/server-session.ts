...
export function createServerSession(
  ...
  return {
    ...
    // UPSTREAM-DIVERGENCE: mobile resume needs a pull refresh of session status because SSE is
    // suspended in the background; upstream only updates session_status from live events.
    // v2 servers project status from the event stream, so the pull applies to v1 only.
    async status() {
