...
function sessionApiWithSessions(sessions: Record<string, Session>, calls: string[]) {
  return {
    get: async ({ sessionID }: { sessionID: string }) => {
      calls.push(sessionID)
      return sessions[sessionID]
    },
  } as unknown as ServerApi["session"]
}
...
describe("warmSessions", () => {
  test("loads missing session ancestors for prompt request trees", async () => {
    ...
    const calls: string[] = []
    const api = sessionApiWithSessions(
      {
    ...
    await warmSessions({ ids: ["grandchild"], store, setStore, api })
