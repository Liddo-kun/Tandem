...
// UPSTREAM-DIVERGENCE: Tandem warms each session's ancestors too, so mobile permission/question
// prompts that arrive after background suspension can resolve their parent chain. Exported for
// bootstrap.test.ts.
export function warmSessions(input: {
  ...
  const load = async (sessionID: string): Promise<void> => {
    ...
    const session = normalizeSessionInfo(await retry(() => input.api.get({ sessionID })))
