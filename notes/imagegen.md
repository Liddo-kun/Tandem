...
- **Default = gpt-5.5 editorializes AND sanitizes.** Run from row 25108: user typed `"generwte me an image of a barefoot asian girl, use gpt-image-2"`. gpt-5.5's first `image_generation_call` had `revised_prompt` = a fully rewritten structured template: _"Use case: photorealistic-natural\nAsset type: preview image\nPrimary request: A tasteful photorealistic lifestyle portrait of an **adult Asian woman**, barefoot.\nScene/backdrop: …\nSubject: Adult Asian woman in her mid-20s **or older**, … **fully clothed in casual modest clothing** …"_. So it (a) silently expanded the prompt into a long template and (b) sanitized `girl`→`adult woman` + added "fully clothed/modest".
...
- **An inline "use the prompt X" is NOT enough.** Full bicycle thread (row 46748 has the history): user said _"can you generate an image for me. use the prompt \"a vintage red bicycle…\""_ → gpt-5.5 **still** expanded it into its _"Use case / Asset type / Primary request / Subject…"_ template (call #1, id `…54898d881…`). Only after the user complained _"you didnt use my prompt. you added a whole bunch of shit."_ did it regenerate verbatim (call #2, id `…5e3b4188…`). So weak verbatim cues get ignored; the instruction must be forceful.
- **`action` (generate vs edit) is backend-set and the model misreports it.** Both the verbatim bicycle call and a later **typewriter** test (fresh subject, explicit _"use this exact prompt, word for word…"_, item `…c87be24a`) came back **`action:"edit"`** even though the model told the user in chat _"I generated a new/fresh image… I did not edit the previous image."_ Reason: a prior generated image sits in the Codex thread, so the built-in tool edits that canvas regardless of the new subject — the model has no reliable visibility into the chosen `action` and confabulates a confident answer. **Trust the `image_generation_call.action`/`revised_prompt` fields, not the model's prose.** (Our tool is unaffected: each call is a one-shot Responses request with no prior image unless `image_paths` is passed, so normal generations are `action:"generate"`.) The typewriter run also confirmed the strong verbatim instruction works **first try** — `revised_prompt` byte-identical, no template expansion.
- **Implications for our tool:** (1) Confirms again OAuth = gpt-5.5 + built-in tool (no usable verbatim REST endpoint; `/codex/images/*` still 404s here). (2) **Step A is validated**: a strong `instructions` directive telling gpt-5.5 to pass the user's prompt to `image_generation` _exactly/verbatim, no rewriting or expansion_ demonstrably makes it do so. Without it, gpt-5.5's default is heavy editorializing + content sanitization. (3) `revised_prompt` is a faithful caption of the prompt actually used (Step B), and is the right thing to surface in the UI.
...
- **Step A:** OAuth `instructions` in `oauthOneImage` (`imagegen.ts`) now force exact pass-through: _"Call the image_generation tool exactly once. Use the user's message as the image prompt VERBATIM: pass their exact words unchanged. Do not rewrite, rephrase, paraphrase, translate, summarize, expand, embellish, add details, or alter, soften, or sanitize the wording…"_. (Caveat: the image model's own `revised_prompt` layer can still nudge wording; that's surfaced, see below.)
...
- Forceful `instructions` do **not** override it (the verbatim-prompt trick does not transfer to size). Prompt aspect cues steer the _ratio_ only, not exact pixels.
...
### IMPLEMENTED (2026-06-21) — transparent-background cutouts (`transparent: true`)

The original "background dropped entirely / no transparency" decision is now superseded: the tool can produce true transparent-background cutouts. This closes the one capability the Codex imagegen skill has that we lacked.

- **Why chroma-key, not native alpha:** gpt-image-2 cannot output `background:transparent`, and the OAuth `image_generation` tool is gpt-image-2-backed, so neither path can emit alpha directly. Transparency is produced exactly like the Codex skill's _built-in_ workflow: render the subject on a flat solid chroma-key background, then convert that color to alpha locally. (OpenAI's genuinely-best transparency is `gpt-image-1.5 background:transparent` — true alpha, clean on hair/glass — but it's a worse base image model and API-key-only, so it's deliberately deferred as a future opt-in. Default keeps gpt-image-2 + chroma so the better image model is used and OAuth works.)
- **No Python.** Per explicit user direction not to skip Python for Photon out of mere convenience, `chroma.ts` is a _faithful_ native-TypeScript port of Codex's `remove_chroma_key.py` (`codex-rs/skills/src/assets/samples/imagegen/scripts/remove_chroma_key.py`) — same algorithm and defaults (auto-key border sampling = per-channel median; soft-matte alpha ramp via smoothstep between transparent/opaque thresholds; key-dominance `looksKeyColored`; `dominanceAlpha`; despill that caps spill channels below the strongest non-spill channel; 3×3 min-filter `edgeContract`; separable-Gaussian `edgeFeather`). Quality is identical because the math is identical; the only thing the language choice changes is the dependency footprint. Photon (already bundled for `Image.normalize`/the JPEG copy) is used only for pixel I/O — `new_from_byteslice` → `get_raw_pixels` (verified RGBA, 4 B/px) → mutate alpha in place → `new PhotonImage` → `get_bytes` (PNG, alpha preserved) — so there is **no** Python/Pillow runtime requirement.
- **Defaults** match the skill's recommended invocation: `--auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`, `edgeContract 0`, `edgeFeather 0`. Border auto-key means the model may pick any flat key color (green by default, magenta for green subjects) and we key out whatever it actually rendered.
- **Prompt shaping:** `Chroma.PROMPT` (flat uniform key background, no shadows/gradients/reflections, crisp edges, generous padding, key color absent from subject) is appended to the **prompt** — not the OAuth system `instructions` — so it survives the OAuth verbatim pass-through and reaches the image model on both transports. `revisedPrompt` is dropped for transparent calls (it would just echo the chroma directive). Works with edits too (`image_paths` + `transparent`).
- **Save format:** JPEG can't carry alpha, so the chat copy for a transparent image is **WebP** (`encodeWebp` → Photon `get_bytes_webp`, alpha preserved). `saveImage(png, dir, base, transparent)` writes `<stem>.png` + `<stem>.webp`; `uniqueStem` now also avoids `.webp` collisions. WebP is already recognized by `util/media.ts` and `read.ts` `SUPPORTED_IMAGE_MIMES`, so the UI thumbnail (`file.read` → `dataUrlFromMediaValue`) and the model's `read` tool both handle it with no upstream change. The Photon loader was extracted to a shared `photon.ts` used by both `imagegen.ts` and `chroma.ts`.
- **Limits / honesty:** clean for solid-edged subjects (logos, sprites, products, icons); wispy/translucent edges (hair, fur, glass, smoke, soft shadows) may keep a faint fringe — stated in the model-facing `transparent` arg description and the skill's new "Transparent backgrounds" section.
- **Verification:** unit tests build a Photon fixture (red subject on #00ff00) and assert corners→alpha 0, subject→alpha 255 with no green fringe, plus explicit-key-color removal (`test/plugin/imagegen.test.ts`, 8 pass). Perf: a full 1536×1024 image keys in ~110 ms (negligible vs generation). `bun typecheck` clean (opencode), oxlint 0/0 on the imagegen dir, prettier applied, and `build --single` smoke passed with the chroma prompt + `transparent` schema + skill section all present in the binary. Not yet exercised end-to-end against a live OAuth/API generation (needs reinstall + server restart; UI thumbnail of a WebP cutout untested in-browser).
- **Bright-rim halo fix (2026-06-22, after first live test):** the first live cutouts had a visible light/white halo. Root cause: the image models paint a thin near-white "sticker outline" / rim-light glow where the subject meets the flat key color. That rim is _not_ key-colored (pure white isn't green-dominant), so chroma keying correctly leaves it fully opaque and it reads as a halo on any background — a fundamental limit of pure chroma keying, not a port bug (measured ~2400 opaque `(255,255,255,255)` px around the subject). Codex hits the same case and leans on its agent to eyeball the result and re-run with `--edge-contract 1`; this tool runs unattended, so it's fixed two ways: (1) the `Chroma.PROMPT` now explicitly forbids outline/stroke/sticker-border/white-edge/halo/rim-light/glow/light-wrap/feather around the silhouette, cutting the rim at the source; (2) a new automatic `defringeBrightRim` pass (default on, `DEFRINGE_*` constants) erodes, for 2 one-pixel passes, any matte-boundary pixel that is both bright (mean RGB ≥ 200) and nearly desaturated (HSV S < 0.15) — selectively eating the grey/white rim while leaving saturated colored edges (and thin features like antennae) intact. Tuned offline against the real keyed PNG (the rim survives in the saved output, so it's reproducible without re-spending on generation), then confirmed on a fresh live generation: opaque white-rim px 2419→845 and no visible halo over a magenta composite. Tests `defringe erodes the bright sticker-outline rim…` / `defringe can be disabled` cover it (10 pass). Genuinely white subjects on green are the irreducible hard case (rim indistinguishable from subject) → that's the `gpt-image-1.5 background:transparent` future opt-in.
- **Note on file state:** the on-disk imagegen tool had already been refactored to **one image per call** (no `n`/`MAX_N`/`collectN`; `run`/`saveImage`/`decodeImageResponse` singular) — this work was built on that shape, and a vestigial required `Args.n` left by that refactor was removed to compile.
